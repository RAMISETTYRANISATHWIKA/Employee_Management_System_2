...
Each developer needs a MongoDB database and their own local `.env` file. Login accounts are stored in MongoDB; they are not stored in GitHub. After configuring `.env`, create local test accounts:

```powershell
npm run seed:all
```

The seeder generates passwords and saves them to `credentials/initial-credentials.txt`. This file is intentionally git-ignored, so each developer must use the credentials generated by their own database. If the team uses one shared MongoDB database, configure the same `MONGO_URI` and use the accounts already created there instead of running the seeder repeatedly.
